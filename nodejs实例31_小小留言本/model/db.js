...
exports.find = function (collectionname, json, C, D) {
    if (arguments.length==4) {//核查参数数量
        ...
        var jumpcount = args.pagecount*(args.page-1) || 0;
        var sort = args.sort || {};      
    } else
    ...
    _connectDB(function (err, dbase) {
        ...
        var cursor = dbase.collection(collectionname).find(json).limit(pagecount).skip(jumpcount).sort(sort);
...
//取得集合的文档数
exports.getcounts = function (collectionname, callback) {
    _connectDB(function (err, dbase) {
        dbase.collection(collectionname).count({}).then(function (count) {
            callback(count);
        });
        db.close();
    })
